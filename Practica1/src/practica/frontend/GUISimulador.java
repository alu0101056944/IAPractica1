...
public class GUISimulador extends GUIWindow{
	...
	public void construirVentana() {
		...
		int longitudCeldas = 30;
		GUIPanelEscenario panelEscenario = new GUIPanelEscenario(simulador.getEscenario().getMatrizCeldas(), longitudCeldas);
		panelEscenario.setBackground(cPExt);
		panelEscenario.setSize(new Dimension(400,400));
		
		JPanel panelBaseEscenario = new JPanel();
		panelBaseEscenario.setBackground(Color.LIGHT_GRAY);
		
		panelBaseEscenario.add(panelEscenario);
		
		JScrollPane scrollPane = new JScrollPane(panelBaseEscenario);
		obtenerContenedor().add(scrollPane, BorderLayout.CENTER);
		...
		Font fEjecutar = new Font(botonEjecutar.getFont().getFontName(), Font.PLAIN, 9);
		botonEjecutar.setFont(fEjecutar);
		botonEjecutar.addActionListener(new EventoEjecutar(simulador, panelEscenario));
		panelMenu.add(botonEjecutar);
		
		JButton botonCrear = new JButton("Crear");
		Font fCrear = new Font(botonCrear.getFont().getFontName(), Font.PLAIN, 9);
		botonCrear.setFont(fCrear);
		EventoCrear eCrear = new EventoCrear(panelBaseEscenario, panelEscenario, panelMenu);
		...
		JMenuItem opcionAbrirArchivo = new JMenuItem("Abrir escenario en archivo");
		EventoAbrirFichero eventoAbrir = new EventoAbrirFichero(panelBaseEscenario, this, panelMenu, panelEscenario);
		opcionAbrirArchivo.addActionListener(eventoAbrir);
		menuArchivo.add(opcionAbrirArchivo);
...
class EventoCrear implements ActionListener{
	...
	public EventoCrear(JPanel panelBase, GUIPanelEscenario panel, GUIPanelMenu menu) {
...
class EventoAbrirFichero implements ActionListener{
	...
	public EventoAbrirFichero(JPanel panelBase, GUISimulador ventanaPadre, GUIPanelMenu panelMenu,
